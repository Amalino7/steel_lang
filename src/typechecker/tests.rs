    ...
    match res {
        Err(errors) => {
            assert!(
                errors
                    .iter()
                    .any(|e| matches!(e, TypeCheckerError::InvalidReturnOutsideFunction { .. }))
            );
        }
...
fn test_tc_multiple_errors() {
    ...
    for error in errors {
        match error {
            ...
            TypeCheckerError::InvalidReturnOutsideFunction { .. } => found_invalid_return = true,
...
fn test_tc_function_unreachable_code() {
    ...
    let mut ast = parser.parse().expect("Parser failed.");
    let mut checker = TypeChecker::new();
    let (_typed_ast, warnings) = checker
        .check(ast.as_mut_slice())
...
fn test_unreachable_code_after_return() {
    ...
    // Both unreachable statements should generate warnings
    assert!(
        warnings
            .iter()
